      ...
      <div className="md:flex flex-wrap mt-5 gap-5">
        {Array.isArray(displayedData) &&
          displayedData.map((item, index) => (
            <div key={index} className="w-full md:w-[455px]">
              <div className="text-[#5b5b5b] border-b-4  border-[#5b5b5b]">
                {item.time}
              </div>
              <div className="">
                <img
                  className="w-full md:w-[453px] object-cover h-[254px] mt-4 -z-50"
                  src={`https://api.abdullajonov.uz/legend-backend-api/public/storage/images/${item.image}`}
                  alt=""
                />
              </div>
              <div className="w-full md:bg-[#fff] text-[#5b5b5b] shadow-2xl md:w-[414px] h-[220px] p-5 mx-auto">
